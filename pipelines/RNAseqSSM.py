...
def RNA_SSM(file_list, s3_output):
	...
	if (len(file_list) % 2) == 0: 
		...
		for pair1 in range(0,len(file_list),2):
			...
			sam_sort = "sudo docker run -v /home/ec2-user/:/data houghtos/immunotools:version2 /usr/local/bin/bin/samtools sort -n -@ 6 -m 4G /data/output/{} -o /data/output/{}".format(out_sam,aligned_out_bam)
			sam_view = "sudo docker run -v /home/ec2-user/:/data houghtos/immunotools:version2 /usr/local/bin/bin/samtools view /data/output/{} -o /data/output/{}".format(aligned_out_bam,aligned_out_sam)
			htseq = "sudo docker run -v /home/ec2-user/:/data houghtos/immunotools:version2 /usr/local/bin/htseq-count --stranded=no --order=name --idattr=gene_name --mode=intersection-nonempty /data/output/{} /data/refs/Mus_musculus.GRCm38.75.gtf > /home/ec2-user/output/{}".format(aligned_out_sam,gene_counts) #<<<<< The .GTF file here is set from the reference files (mm10 example here)
